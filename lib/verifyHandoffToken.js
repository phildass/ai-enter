...
const ALLOWED_APPS = ['jai-bharat', 'jai-kisan', 'appmall', 'iisacademy'];
const ALLOWED_RETURN_DOMAINS = ['jaibharat.cloud', 'jaikisan.cloud', 'appmall.in', 'iisacademy.in'];
const EXPECTED_AMOUNT_PAISE = 11800;
const LEGACY_INTRO_AMOUNT_PAISE = 11682;
...
export function verifyHandoffToken(token, options = {}) {
  ...
  if (payload.app_name === 'iisacademy') {
    ...
    payload.amount_paise !== EXPECTED_AMOUNT_PAISE &&
    payload.amount_paise !== LEGACY_INTRO_AMOUNT_PAISE
  ) {
    throw new Error('Invalid amount in token');
  }

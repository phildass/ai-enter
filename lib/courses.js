 ...
 */
export function resolveAppmallAmountPaise(courseSlug, amountInrFromQuery) {
  ...
  if (amountInrFromQuery != null && amountInrFromQuery !== '') {
    ...
    if (Number.isFinite(n) && n > 0) {
      ...
      // Accept live membership or legacy intro only if still passed (grace for in-flight links).
      if (paise === APPMALL_DEFAULT_AMOUNT_PAISE || paise === ASTRO_QUESTION_AMOUNT_PAISE) {
        return paise;
      }

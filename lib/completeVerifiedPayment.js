...
import { getAppmallDefaultAmountPaise, isInternationalPaymentCourse } from './courses';
...
export async function completeVerifiedPayment(input) {
  ...
  if (appmallPayload) {
    ...
    const international = isInternationalPaymentCourse(appId) || appmallPayload.offer === 'international';
    const amountPaise = international
      ? Number(
          appmallPayload.amount_cents ||
            appmallPayload.amount_paise ||
            appmallPayload.amountPaise ||
            0,
        )
      : appmallPayload.amount_paise || appmallPayload.amountPaise || getAppmallDefaultAmountPaise();
    if (international && !amountPaise) {
      return {
        ok: false,
        status: 400,
        error: 'International payment token is missing a valid amount',
      };
    }
    const confirmUrl =
    ...
    if (!confirmResult.ok) {
      ...
      const fallbackParams = new URLSearchParams({
        ...
        user_name: appmallPayload.name || appmallPayload.user_name || appmallPayload.customer_name || '',

...
export default async function handler(req, res) {
  ...
  if (iiskills_token) {
    ...
    try {
      ...
      return res.status(400).json({ error: err.message || 'Invalid uriq token' });
    }
  ...
  try {
    ...
    if (!keySecret) {
      return res.status(500).json({ error: `Payment system not configured for ${app_name}` });
    }
    ...
    if (keyId) {
      try {
        ...
        console.error(
          '[verify-payment] Failed to fetch payment details from Razorpay (non-fatal):',
          err.message,
        );
      }
    }
    ...
    if (supabaseUrl && serviceRoleKey) {
      ...
      if (error) {
        console.error('[verify-payment] Supabase update failed (non-fatal):', error.message);
      } else {
    ...
    if (iiskillsPayload || uriqPayload) {
      ...
      if (!confirmResult.ok) {
        console.error(
          `[verify-payment] ${app_name} confirm failed: purchaseId=${payload.purchaseId} paymentId=${razorpay_payment_id} error=${confirmResult.error}`,
        );
        if (confirmResult.debug) {
          console.error(`[verify-payment] ${app_name} confirm debug:`, confirmResult.debug);
        }
